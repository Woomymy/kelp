...
pub fn get_root_files() -> anyhow::Result<Vec<FileInfo>> {
    ...
    let config_yaml: Vec<FileInfo> = serde_yaml::from_str(conf_bundled).with_context(|| red!("Unable to parse bundled autoconfig root file!"))?;
...
pub fn get_home_files() -> anyhow::Result<Vec<FileInfo>> {
    ...
    let config_yaml: Vec<FileInfo> = serde_yaml::from_str(conf_bundled).with_context(|| red!("Unable to parse bundled autocofig home file!"))?;
    for file in config_yaml {
